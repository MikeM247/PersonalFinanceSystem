...
import { applySqlMigrationFile, createNodeSqliteExecutor, hashPassword, SqlAuditEventRepository, SqlUserRepository, SqlUserSessionRepository, } from "#db/src/index.js";
import { AuthService, hashSessionToken } from "#core/src/index.js";
import { createAuthApiHandlersWithRateLimiter, InMemoryLoginAttemptRateLimiter, } from "../src/index.js";
...
async function createApiHarness(now = new Date("2026-04-05T14:00:00.000Z")) {
    ...
    return {
        ...
        authService,
        telemetry,
        handlers: createAuthApiHandlersWithRateLimiter(authService, configuration, new InMemoryLoginAttemptRateLimiter(configuration.loginWindowMinutes, configuration.loginMaxAttempts), {
            auditRepository: repositories.auditEvents,
            telemetry,
            now: () => now,
            generateAuditEventId: (() => {
...
function listAuditEvents(executor) {
    return executor.query(`SELECT event_type, request_id, metadata
     FROM audit_events
     ORDER BY occurred_at ASC, id ASC`).rows;
}
function getCookieValue(cookieValue) {
    assert.ok(cookieValue);
    return cookieValue;
}
async function testLoginEndpointSuccess() {
    ...
    const persisted = await harness.repositories.sessions.findByTokenHash(hashSessionToken(getCookieValue(response.cookies?.[0]?.value), harness.configuration.sessionSecret));
    assert.ok(persisted);
    assert.ok(harness.telemetry.counters.some((entry) => entry.name === "auth_login_success_total"));
...
async function testLoginEndpointValidationAndUnauthorizedResponses() {
    const harness = await createApiHarness();
    ...
    assert.ok(harness.telemetry.counters.some((entry) => entry.name === "auth_login_failure_total"));
}
async function testSessionEndpointAndLogout() {
    const harness = await createApiHarness();
...
async function testAuthenticatedGuard() {
    const harness = await createApiHarness();
    const login = await harness.handlers.login({
        body: {
            email: "owner@example.com",
    ...
    assert.ok(harness.telemetry.counters.some((entry) => entry.name === "auth_unauthorized_request_total"));
...
async function testLoginThrottling() {
    const harness = await createApiHarness();
    const telemetry = createTelemetryRecorder();
    const handlers = createAuthApiHandlersWithRateLimiter(harness.authService, harness.configuration, new InMemoryLoginAttemptRateLimiter(15, 2), {
    ...
    assert.ok(telemetry.counters.some((entry) => entry.name === "auth_login_rate_limited_total"));
}
async function testExpiredSessionGuardRejection() {
    ...
    const handlers = createAuthApiHandlersWithRateLimiter(authService, harness.configuration, new InMemoryLoginAttemptRateLimiter(harness.configuration.loginWindowMinutes, harness.configuration.loginMaxAttempts), {
        auditRepository: harness.repositories.auditEvents,
        telemetry: harness.telemetry,
        now: () => currentTime,
        generateAuditEventId: (() => {
    ...
    assert.ok(harness.telemetry.logs.some((entry) => entry.eventType === "auth.access.denied"));
}
async function main() {
